...
paramiko.util.log_to_file("paramiko_patch.log")
...
try:
    ...
    # Files to deploy
    files_to_deploy = [
        ("src/app/api/stripe/checkout/route.ts", "/root/fmcg-platform/src/app/api/stripe/checkout/route.ts"),
    ]
    ...
    # Rebuild
    print("Triggering Rebuild (Quick)...")
    # Just killing the process might work if dev mode, but this is prod. 
    # Must rebuild for API route changes in Next.js? usually yes.
